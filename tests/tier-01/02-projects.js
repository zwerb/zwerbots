...
describe("Tier One: Projects", () => {
  ...
  describe("Sequelize Model", () => {
    ...
    it("*** deadline must be a valid date", async () => {
      // No date provided is fine, it will hit the default
      delete project.deadline;
      const noDate = Project.build(project);
      try {
        await noDate.validate();
        throw Error("Validation should not have failed due to default");
      } catch (err) {
        expect(err.message).to.contain("Validation should not have failed");
      }

      // No date provided is fine, it will hit the default
      project.deadline = "";
      const emptyDate = Project.build(project);
      try {
        await emptyDate.validate();
        throw Error("Validation should have failed with empty date string");
      } catch (err) {
        expect(err.message).to.not.contain("Validation should have failed");
      }

      // No date provided is fine, it will hit the default
      project.deadline = "A349";
      const badDate = Project.build(project);
      try {
        await badDate.validate();
        throw Error("Validation should have failed with bad date string");
      } catch (err) {
        expect(err.message).to.not.contain("Validation should have failed");
      }
      ...
      const nullDate = Project.build(project);
      try {
        await nullDate.validate();
        throw Error("Validation should have failed with null date string");
      } catch (err) {
        expect(err.message).to.not.contain("Validation should have failed");
      }
    });
